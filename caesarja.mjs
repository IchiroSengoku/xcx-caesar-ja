var img$2 = "data:image/png;base64,";

var img$1 = "data:image/svg+xml,%3c%3fxml version='1.0' encoding='UTF-8' standalone='no'%3f%3e%3c!DOCTYPE svg PUBLIC '-//W3C//DTD SVG 1.1//EN' 'http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd'%3e%3csvg width='100%25' height='100%25' viewBox='0 0 53 53' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' xml:space='preserve' xmlns:serif='http://www.serif.com/' style='fill-rule:evenodd%3bclip-rule:evenodd%3bstroke-linejoin:round%3bstroke-miterlimit:2%3b'%3e%3crect id='%e3%82%a2%e3%83%bc%e3%83%88%e3%83%9c%e3%83%bc%e3%83%891' x='0' y='-0' width='52.083' height='52.056' style='fill:none%3b'/%3e%3cclipPath id='_clip1'%3e%3crect x='0' y='-0' width='52.083' height='52.056'/%3e%3c/clipPath%3e%3cg clip-path='url(%23_clip1)'%3e%3cg%3e%3cg%3e%3cpath d='M17.605%2c14.113c1.125%2c0.983 1.582%2c2.148 2.563%2c3.17c2.407%2c2.507 4.811%2c5.102 7.423%2c7.551c4.352%2c4.079 10.045%2c7.577 14.675%2c11.435' style='fill:none%3bstroke:white%3bstroke-width:19.17px%3bstroke-linecap:round%3bstroke-miterlimit:1.5%3b'/%3e%3cpath d='M40.824%2c11.292c-0.914%2c2.03 -3.668%2c3.898 -5.723%2c5.559c-2.684%2c2.171 -5.276%2c4.398 -7.941%2c6.581c-7.278%2c5.958 -12.955%2c12.359 -18.301%2c19.04' style='fill:none%3bstroke:white%3bstroke-width:19.17px%3bstroke-linecap:round%3bstroke-miterlimit:1.5%3b'/%3e%3c/g%3e%3cg%3e%3cpath d='M17.605%2c14.113c1.125%2c0.983 1.582%2c2.148 2.563%2c3.17c2.407%2c2.507 4.811%2c5.102 7.423%2c7.551c4.352%2c4.079 10.045%2c7.577 14.675%2c11.435' style='fill:none%3bstroke:%23f9a83a%3bstroke-width:12.5px%3bstroke-linecap:round%3bstroke-miterlimit:1.5%3b'/%3e%3cpath d='M40.824%2c11.292c-0.914%2c2.03 -3.668%2c3.898 -5.723%2c5.559c-2.684%2c2.171 -5.276%2c4.398 -7.941%2c6.581c-7.278%2c5.958 -12.955%2c12.359 -18.301%2c19.04' style='fill:none%3bstroke:%23f9a83a%3bstroke-width:12.5px%3bstroke-linecap:round%3bstroke-miterlimit:1.5%3b'/%3e%3c/g%3e%3cg%3e%3cpath d='M16.783%2c15.055c0.264%2c0.231 0.482%2c0.475 0.679%2c0.725c0.621%2c0.785 1.078%2c1.612 1.804%2c2.368c1.456%2c1.517 2.91%2c3.065 4.41%2c4.594c0.996%2c1.015 2.012%2c2.021 3.06%2c3.004c1.353%2c1.268 2.831%2c2.481 4.368%2c3.666c3.429%2c2.643 7.152%2c5.143 10.362%2c7.817c0.53%2c0.442 1.319%2c0.37 1.76%2c-0.16c0.442%2c-0.53 0.37%2c-1.319 -0.16%2c-1.76c-3.233%2c-2.694 -6.982%2c-5.215 -10.435%2c-7.877c-1.472%2c-1.135 -2.89%2c-2.296 -4.185%2c-3.51c-1.023%2c-0.959 -2.014%2c-1.941 -2.985%2c-2.931c-1.494%2c-1.522 -2.942%2c-3.064 -4.392%2c-4.574c-0.667%2c-0.695 -1.075%2c-1.465 -1.645%2c-2.187c-0.288%2c-0.365 -0.61%2c-0.72 -0.997%2c-1.058c-0.52%2c-0.454 -1.31%2c-0.4 -1.764%2c0.119c-0.453%2c0.52 -0.4%2c1.31 0.12%2c1.764Z' style='fill:white%3b'/%3e%3cpath d='M39.685%2c10.779c-0.331%2c0.733 -0.963%2c1.428 -1.694%2c2.108c-1.138%2c1.059 -2.52%2c2.058 -3.676%2c2.992l-7.947%2c6.585c-3.596%2c2.944 -6.805%2c5.995 -9.778%2c9.133c-3.104%2c3.276 -5.949%2c6.648 -8.707%2c10.094c-0.431%2c0.539 -0.343%2c1.326 0.195%2c1.757c0.539%2c0.431 1.326%2c0.344 1.757%2c-0.195c2.715%2c-3.392 5.515%2c-6.712 8.57%2c-9.937c2.903%2c-3.064 6.036%2c-6.043 9.547%2c-8.917l7.935%2c-6.576c1.418%2c-1.146 3.155%2c-2.393 4.43%2c-3.716c0.716%2c-0.743 1.293%2c-1.516 1.647%2c-2.301c0.283%2c-0.629 0.003%2c-1.37 -0.626%2c-1.653c-0.629%2c-0.284 -1.37%2c-0.003 -1.653%2c0.626Z' style='fill:white%3b'/%3e%3c/g%3e%3c/g%3e%3c/g%3e%3c/svg%3e";

var en$1 = {
	"caesarja.entry.name": "Caesar JA",
	"caesarja.entry.description": "Description of this extension"
};
var ja$1 = {
	"caesarja.entry.name": "Caesar JA",
	"caesarja.entry.description": "拡張機能の説明"
};
var translations$1 = {
	en: en$1,
	ja: ja$1,
	"ja-Hira": {
	"caesarja.entry.name": "Caesar JA",
	"caesarja.entry.description": "かくちょうきのうのせつめい"
}
};

var version$1 = "0.0.0";

/**
 * This is an extension for Xcratch.
 */


/**
 * Formatter to translate the messages in this extension.
 * This will be replaced which is used in the React component.
 * @param {object} messageData - data for format-message
 * @returns {string} - translated message for the current locale
 */
var formatMessage$1 = function formatMessage(messageData) {
  return messageData.defaultMessage;
};
var version = "v".concat(version$1);
var entry = {
  get name() {
    return formatMessage$1({
      id: 'caesarja.entry.name',
      defaultMessage: 'シーザー暗号',
      description: 'name of the extension'
    });
  },
  extensionId: 'caesarja',
  extensionURL: 'https://IchiroSengoku.github.io/xcx-caesar-ja/dist/caesarja.mjs',
  collaborator: 'IchiroSengoku',
  iconURL: img$2,
  insetIconURL: img$1,
  get description() {
    return "".concat(formatMessage$1({
      defaultMessage: 'an extension for Xcratch',
      description: 'Description for this extension',
      id: 'caesarja.entry.description'
    }), " (").concat(version, ")");
  },
  tags: [],
  featured: true,
  disabled: false,
  bluetoothRequired: false,
  internetConnectionRequired: false,
  helpLink: 'https://IchiroSengoku.github.io/xcx-caesar-ja/',
  setFormatMessage: function setFormatMessage(formatter) {
    formatMessage$1 = formatter;
  },
  translationMap: translations$1
};

function _classCallCheck(a, n) {
  if (!(a instanceof n)) throw new TypeError("Cannot call a class as a function");
}

function _typeof(o) {
  "@babel/helpers - typeof";

  return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) {
    return typeof o;
  } : function (o) {
    return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o;
  }, _typeof(o);
}

function toPrimitive(t, r) {
  if ("object" != _typeof(t) || !t) return t;
  var e = t[Symbol.toPrimitive];
  if (void 0 !== e) {
    var i = e.call(t, r);
    if ("object" != _typeof(i)) return i;
    throw new TypeError("@@toPrimitive must return a primitive value.");
  }
  return (String )(t);
}

function toPropertyKey(t) {
  var i = toPrimitive(t, "string");
  return "symbol" == _typeof(i) ? i : i + "";
}

function _defineProperties(e, r) {
  for (var t = 0; t < r.length; t++) {
    var o = r[t];
    o.enumerable = o.enumerable || false, o.configurable = true, "value" in o && (o.writable = true), Object.defineProperty(e, toPropertyKey(o.key), o);
  }
}
function _createClass(e, r, t) {
  return r && _defineProperties(e.prototype, r), t && _defineProperties(e, t), Object.defineProperty(e, "prototype", {
    writable: false
  }), e;
}

/**
 * Minimal BlockType for Scratch/Xcratch extension builds.
 * Matches common constants used by extension definitions.
 */
var BlockType = {
  REPORTER: 'reporter'};

/**
 * Minimal ArgumentType for Scratch/Xcratch extension builds.
 * Used in getInfo() to declare input types.
 */
var ArgumentType = {
  STRING: 'string',
  NUMBER: 'number'};

/**
 * Minimal Cast implementation for Xcratch extension builds.
 * Provides commonly used conversions in Scratch VM extensions.
 */
var Cast = {
  toNumber: function toNumber(value) {
    // Scratch-like number conversion: empty or null -> 0
    if (value === null || value === undefined) return 0;
    if (typeof value === 'number') return value;
    var n = Number(value);
    return Number.isNaN(n) ? 0 : n;
  },
  toString: function toString(value) {
    if (value === null || value === undefined) return '';
    return String(value);
  },
  toBoolean: function toBoolean(value) {
    // Scratch-like truthiness (roughly): 0, "0", "", null, undefined -> false
    if (value === null || value === undefined) return false;
    if (typeof value === 'boolean') return value;
    if (typeof value === 'number') return value !== 0;
    var s = String(value).toLowerCase();
    if (s === '' || s === '0' || s === 'false') return false;
    return true;
  }
};

var en = {
	"caesarja.name": "Caesar JA",
	"caesarja.doIt": "do it [SCRIPT]"
};
var ja = {
	"caesarja.name": "シーザー暗号",
	"caesarja.doIt": "[SCRIPT] を実行する"
};
var translations = {
	en: en,
	ja: ja,
	"ja-Hira": {
	"caesarja.name": "シーザー　あんごう",
	"caesarja.doIt": "[SCRIPT] をじっこうする"
}
};

var img = "data:image/png;base64,";

function _createForOfIteratorHelper(r, e) { var t = "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"]; if (!t) { if (Array.isArray(r) || (t = _unsupportedIterableToArray(r)) || e) { t && (r = t); var _n = 0, F = function F() {}; return { s: F, n: function n() { return _n >= r.length ? { done: true } : { done: false, value: r[_n++] }; }, e: function e(r) { throw r; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var o, a = true, u = false; return { s: function s() { t = t.call(r); }, n: function n() { var r = t.next(); return a = r.done, r; }, e: function e(r) { u = true, o = r; }, f: function f() { try { a || null == t.return || t.return(); } finally { if (u) throw o; } } }; }
function _unsupportedIterableToArray(r, a) { if (r) { if ("string" == typeof r) return _arrayLikeToArray(r, a); var t = {}.toString.call(r).slice(8, -1); return "Object" === t && r.constructor && (t = r.constructor.name), "Map" === t || "Set" === t ? Array.from(r) : "Arguments" === t || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(t) ? _arrayLikeToArray(r, a) : void 0; } }
function _arrayLikeToArray(r, a) { (null == a || a > r.length) && (a = r.length); for (var e = 0, n = Array(a); e < a; e++) n[e] = r[e]; return n; }

/**
 * Formatter which is used for translation.
 * This will be replaced which is used in the runtime.
 * @param {object} messageData - format-message object
 * @returns {string} - message for the locale
 */
var formatMessage = function formatMessage(messageData) {
  return messageData.default;
};

/**
 * Setup format-message for this extension.
 */
var setupTranslations = function setupTranslations() {
  var localeSetup = formatMessage.setup();
  if (localeSetup && localeSetup.translations[localeSetup.locale]) {
    Object.assign(localeSetup.translations[localeSetup.locale], translations[localeSetup.locale]);
  }
};
var EXTENSION_ID = 'caesarja';

/**
 * URL to get this extension as a module.
 * When it was loaded as a module, 'extensionURL' will be replaced a URL which is retrieved from.
 * @type {string}
 */
var extensionURL = 'https://IchiroSengoku.github.io/xcx-caesar-ja/dist/caesarja.mjs';

// ===== Kana tables (hiragana/katakana) =====
var HIRA = ["あ", "い", "う", "え", "お", "か", "き", "く", "け", "こ", "さ", "し", "す", "せ", "そ", "た", "ち", "つ", "て", "と", "な", "に", "ぬ", "ね", "の", "は", "ひ", "ふ", "へ", "ほ", "ま", "み", "む", "め", "も", "や", "ゆ", "よ", "ら", "り", "る", "れ", "ろ", "わ", "を", "ん",
// 濁音
"が", "ぎ", "ぐ", "げ", "ご", "ざ", "じ", "ず", "ぜ", "ぞ", "だ", "ぢ", "づ", "で", "ど", "ば", "び", "ぶ", "べ", "ぼ",
// 半濁音
"ぱ", "ぴ", "ぷ", "ぺ", "ぽ",
// 小文字
"ぁ", "ぃ", "ぅ", "ぇ", "ぉ", "ゃ", "ゅ", "ょ", "っ", "ゎ",
// その他
"ゔ"];
var KATA = ["ア", "イ", "ウ", "エ", "オ", "カ", "キ", "ク", "ケ", "コ", "サ", "シ", "ス", "セ", "ソ", "タ", "チ", "ツ", "テ", "ト", "ナ", "ニ", "ヌ", "ネ", "ノ", "ハ", "ヒ", "フ", "ヘ", "ホ", "マ", "ミ", "ム", "メ", "モ", "ヤ", "ユ", "ヨ", "ラ", "リ", "ル", "レ", "ロ", "ワ", "ヲ", "ン",
// 濁音
"ガ", "ギ", "グ", "ゲ", "ゴ", "ザ", "ジ", "ズ", "ゼ", "ゾ", "ダ", "ヂ", "ヅ", "デ", "ド", "バ", "ビ", "ブ", "ベ", "ボ",
// 半濁音
"パ", "ピ", "プ", "ペ", "ポ",
// 小文字
"ァ", "ィ", "ゥ", "ェ", "ォ", "ャ", "ュ", "ョ", "ッ", "ヮ",
// その他
"ヴ"];

// ===== helpers =====
function mod(n, m) {
  return (n % m + m) % m;
}

// 全角数字→半角数字（SHIFT入力が全角になる事故を防ぐ）
function toHalfWidthNumber(value) {
  var s = Cast.toString(value);
  var half = s.replace(/[０-９]/g, function (c) {
    return String.fromCharCode(c.charCodeAt(0) - 0xFEE0);
  });
  var n = Number(half);
  return Number.isNaN(n) ? 0 : n;
}

// 文字（1 code point）を「範囲内で」循環シフト
function shiftCodePointInRange(cp, start, end, shift) {
  var size = end - start + 1;
  var s = mod(shift, size);
  return start + mod(cp - start + s, size);
}
function shiftInSet(ch, set, shift) {
  var idx = set.indexOf(ch);
  if (idx === -1) return null;
  var s = mod(shift, set.length);
  return set[mod(idx + s, set.length)];
}
function shiftAsciiLetter(ch, shift) {
  var cp = ch.codePointAt(0);
  var s = mod(Number(shift), 26);

  // A-Z
  if (cp >= 0x41 && cp <= 0x5A) {
    return String.fromCodePoint(0x41 + mod(cp - 0x41 + s, 26));
  }
  // a-z
  if (cp >= 0x61 && cp <= 0x7A) {
    return String.fromCodePoint(0x61 + mod(cp - 0x61 + s, 26));
  }
  return null;
}
function shiftDigit(ch, shift) {
  var cp = ch.codePointAt(0);
  var s = mod(Number(shift), 10);
  if (cp >= 0x30 && cp <= 0x39) {
    return String.fromCodePoint(0x30 + mod(cp - 0x30 + s, 10));
  }
  return null;
}

/**
 * “全部ずらす”版：
 * - 英字 A-Z / a-z（26）
 * - 数字 0-9（10）
 * - ひらがな・カタカナ（配列内で循環）
 * - 漢字（CJK統合漢字 基本面：U+4E00〜U+9FFF を循環）
 * それ以外はそのまま
 */
function caesarJA(text, shift) {
  var s = Number(shift) || 0;
  var out = '';

  // for..of は Unicode code point 単位で回る（サロゲートペアも安全）
  var _iterator = _createForOfIteratorHelper(String(text)),
    _step;
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      var ch = _step.value;
      // 英字
      var en = shiftAsciiLetter(ch, s);
      if (en !== null) {
        out += en;
        continue;
      }

      // 数字
      var dg = shiftDigit(ch, s);
      if (dg !== null) {
        out += dg;
        continue;
      }

      // ひらがな
      var h = shiftInSet(ch, HIRA, s);
      if (h !== null) {
        out += h;
        continue;
      }

      // カタカナ
      var k = shiftInSet(ch, KATA, s);
      if (k !== null) {
        out += k;
        continue;
      }

      // 漢字（CJK統合漢字 基本面）
      var cp = ch.codePointAt(0);
      if (cp >= 0x4E00 && cp <= 0x9FFF) {
        var shifted = shiftCodePointInRange(cp, 0x4E00, 0x9FFF, s);
        out += String.fromCodePoint(shifted);
        continue;
      }

      // その他はそのまま
      out += ch;
    }
  } catch (err) {
    _iterator.e(err);
  } finally {
    _iterator.f();
  }
  return out;
}

/**
 * Scratch 3.0 blocks for Caesar JA extension (kana/latin/digit/kanji supported).
 */
var ExtensionBlocks = /*#__PURE__*/function () {
  /**
   * Construct a set of blocks for Caesar JA.
   * @param {Runtime} runtime - the Scratch 3.0 runtime.
   */
  function ExtensionBlocks(runtime) {
    _classCallCheck(this, ExtensionBlocks);
    /**
     * The Scratch 3.0 runtime.
     * @type {Runtime}
     */
    this.runtime = runtime;
    if (runtime.formatMessage) {
      // Replace 'formatMessage' to a formatter which is used in the runtime.
      formatMessage = runtime.formatMessage;
    }
  }

  /**
   * @returns {object} metadata for this extension and its blocks.
   */
  return _createClass(ExtensionBlocks, [{
    key: "getInfo",
    value: function getInfo() {
      setupTranslations();
      return {
        id: ExtensionBlocks.EXTENSION_ID,
        name: ExtensionBlocks.EXTENSION_NAME,
        extensionURL: ExtensionBlocks.extensionURL,
        blockIconURI: img,
        showStatusButton: false,
        blocks: [{
          opcode: 'encode',
          func: 'encode',
          blockType: BlockType.REPORTER,
          blockAllThreads: false,
          text: formatMessage({
            id: 'caesarja.encode',
            default: '[TEXT] を [SHIFT] 文字ずらす',
            description: 'encode text with shift'
          }),
          arguments: {
            TEXT: {
              type: ArgumentType.STRING,
              defaultValue: 'こんにちはABC123'
            },
            SHIFT: {
              type: ArgumentType.NUMBER,
              defaultValue: 3
            }
          }
        }, {
          opcode: 'decode',
          func: 'decode',
          blockType: BlockType.REPORTER,
          blockAllThreads: false,
          text: formatMessage({
            id: 'caesarja.decode',
            default: '[TEXT] を [SHIFT] 文字もどす',
            description: 'decode text with shift'
          }),
          arguments: {
            TEXT: {
              type: ArgumentType.STRING,
              defaultValue: 'こんにちはABC123'
            },
            SHIFT: {
              type: ArgumentType.NUMBER,
              defaultValue: 3
            }
          }
        }],
        menus: {}
      };
    }
  }, {
    key: "encode",
    value: function encode(args) {
      var text = Cast.toString(args.TEXT);
      var shift = toHalfWidthNumber(args.SHIFT);
      return caesarJA(text, shift);
    }
  }, {
    key: "decode",
    value: function decode(args) {
      var text = Cast.toString(args.TEXT);
      var shift = toHalfWidthNumber(args.SHIFT);
      return caesarJA(text, -shift);
    }
  }], [{
    key: "formatMessage",
    set:
    /**
     * A translation object which is used in this class.
     * @param {FormatObject} formatter - translation object
     */
    function set(formatter) {
      formatMessage = formatter;
      if (formatMessage) setupTranslations();
    }

    /**
     * @return {string} - the name of this extension.
     */
  }, {
    key: "EXTENSION_NAME",
    get: function get() {
      return formatMessage({
        id: 'caesarja.name',
        default: 'シーザー暗号',
        description: 'name of the extension'
      });
    }

    /**
     * @return {string} - the ID of this extension.
     */
  }, {
    key: "EXTENSION_ID",
    get: function get() {
      return EXTENSION_ID;
    }

    /**
     * URL to get this extension.
     * @type {string}
     */
  }, {
    key: "extensionURL",
    get: function get() {
      return extensionURL;
    }

    /**
     * Set URL to get this extension.
     * The extensionURL will be changed to the URL of the loading server.
     * @param {string} url - URL
     */,
    set: function set(url) {
      extensionURL = url;
    }
  }]);
}();

export { ExtensionBlocks as blockClass, entry };
//# sourceMappingURL=caesarja.mjs.map
